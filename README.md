...
[![Documentation](https://img.shields.io/badge/docs-online-blue)](https://samai-sdk.in/)
...
Per-user long-term memory backed by a Neo4j knowledge graph, instead of replaying the
full conversation transcript on every turn. A private memory agent autonomously writes
facts to each user's graph in the background; your main agent only ever sees a plain-text
summary of what's relevant, never raw graph access.

Requires the optional `neo4j-driver` peer dependency (dynamically imported, same pattern
as `RedisSessionStore`/`ioredis` — the rest of the SDK works fine without it installed):
...
```ts
import { createClient, anthropic, defineAgent, createSession, InMemorySessionStore, enableGraphMemory, chatWithMemory, ensureGraphConstraints } from "samai-sdk";
...
**Serving many users** — `enableGraphMemory({ creds })` creates its own driver, fine for
one user, wasteful for many. `createGraphMemoryManager()` shares a single driver (and its
connection pool) across every user instead:
...
// call per incoming request — creates on first contact, reuses (same driver, same
// background sweep timer) after
...
**Timestamped facts and contradiction handling.** The memory agent writes ordinary facts
through `upsert_fact` (wired in automatically — nothing to configure), which guarantees
timestamping and can retire a contradicting old fact in the same call — "loves hiking"
written in March doesn't just sit forever next to "doesn't hike anymore" written in
August:
...
Fade old, unreinforced facts and prune anything past a threshold with `applyRecencyDecay()`:
...
**Self-correction.** `runSelfCorrection()` / `startSelfCorrectionLoop()` run real Cypher
diagnostics (duplicate nodes, overly generic relationship types, relationship-count
overload) and only invoke a curator agent — with the specific findings, not the whole
graph — when there's something to actually fix:

```ts
import { startSelfCorrectionLoop } from "samai-sdk";

const selfCorrection = startSelfCorrectionLoop({ memory, maxRelationships: 200 }); // default 15 min interval
```

**Feed ranking.** `createFeedEngine()` ranks content with a hybrid score — social
proximity (who you follow/are friends with), interest-graph affinity (pulled from the
same memory graph `upsert_fact` writes to), and log-scaled engagement — rather than pure
follow-graph or raw like-count ranking:
...
**Right-to-be-forgotten** and **DB-level uniqueness constraints** (stop duplicate-node
races at the source, rather than cleaning them up after):

```ts
import { ensureGraphConstraints, deleteUserGraph } from "samai-sdk";

await ensureGraphConstraints(await memory.driverPromise); // run every deploy, idempotent
const result = await deleteUserGraph(await memory.driverPromise, "user-123", { deep: true });
```

**Observability.** `createMetricsCollector()` is a single collector every function above
accepts as an optional `metrics` param — sweep success rate & latency, how often
self-correction finds problems, decay/prune volume, feed query latency — aggregated in
one place, not wired to any specific dashboard:

```ts
import { createMetricsCollector } from "samai-sdk";

const metrics = createMetricsCollector();
enableGraphMemory({ /* ... */, metrics });
...
src/
  ...
  optional-deps.d.ts       # ambient types for optional peer deps (ioredis, better-sqlite3, neo4j-driver)
  providers/
  ...
  graph-memory/
    graph-memory.ts           # enableGraphMemory() — per-user Neo4j knowledge graph + background sweep
    fact-lifecycle.ts          # upsert_fact tool (timestamped, contradiction-aware) + applyRecencyDecay()
    self-correction.ts         # runSelfCorrection() / startSelfCorrectionLoop() — dupe/flat-relation cleanup
    feed-engine.ts              # createFeedEngine() — hybrid social+interest+engagement ranking
    admin.ts                   # ensureGraphConstraints() + deleteUserGraph() (right-to-be-forgotten)
    metrics.ts                 # createMetricsCollector() — shared observability across the modules above
    manager.ts                  # createGraphMemoryManager() — one driver shared across many users
    with-running-context.ts     # chatWithMemory() — injects running context into your main agent's turn
  guardrails/
    pii.ts
    prompt-injection.ts
    blocklist.ts
...
npm run example:graph-memory-mock-test                          # enableGraphMemory() sweep -> tool call -> fake driver, asserted end to end
npm run example:graph-memory-self-correction-mock-test           # plants dupes/generic-rels/overload, curator fix reaches the driver
npm run example:graph-memory-self-correction-clean-mock-test     # clean graph correctly skips the curator entirely
npm run example:graph-memory-feed-engine-mock-test                # hybrid ranking: social+interest match outranks raw virality
npm run example:graph-memory-admin-mock-test                       # DB constraints + deleteUserGraph(), shallow and deep modes
